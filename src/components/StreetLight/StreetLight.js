...
class StreetLight extends Component {
  ...
  deleteLight = () => {
    this.store.deleteLight(this.state.selectedIndex).then(() => {
      this.setState({
        selectedIndex: 0
      }, this.getLights)
  ...
  render() {
    ...
    const { visible, selectedIndex, realDateType, unit, type, showType, lightType, mapType } = this.state
    const selectedDevice = lights[selectedIndex] || {}

    return (
      <div className='lightRoot'>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <div className="lightTitle">
            智慧路灯
            <img alt="" onClick={() => this.onClickShowType('map')} height={20} width={20} src={map} />
          </div>
          {isAdmin && <div>
            <Popconfirm placement="bottom" title="确认删除" onConfirm={() => this.deleteLight()} okText="确定" cancelText="取消">
          ...
          <div>
            {lights.map((item, index) => (
              <div className={`device ${selectedIndex === index ? 'light selectedDevice ' : 'light'}`} key={item.id}>
                ...
                <img alt="" onClick={() => this.onClickShowType('attribute', index)} height={30} width={30} src={attributeIcon} />
                <img alt="" onClick={() => this.onClickShowType('map', index)} height={20} width={20} src={map} />
              </div>
            ))}
          </div>
          {
              ...
              <div className="lightSection">
                <div>
                  ...
                  <div className='section'>
                    ...
                    <img alt="" width='20' src={realTimeData.NET === '1' ? wifi : wifiClose} className="lightStatus" />
                  </div>
                  <div className='section'>
                  ...
                  <div className='section'>
                    <h3 className="sectionTitle">喷雾器控制<img alt="" width='20' style={{ marginLeft: 10 }} src={realTimeData.SPY === 1 ? open : close} className="lightStatus" /></h3>
                    <Button
